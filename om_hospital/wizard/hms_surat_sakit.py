...
class CreateSuratSakitWiz(models.TransientModel):
    _name = "create.suratsakit.wizard"
    # _inherit = ["hms.appointment"]
    _description = "Create SuratSakit Wizard"
    ...
    sick_day = fields.Integer(string='Lama Sakit', tracking=True, required=True)
    needed = fields.Char(string='Keperluan', required=True, tracking=True)

    @api.model
    def default_get(self, fields):
        # self.state = 'confirm'
        result = super(CreateSuratSakitWiz, self).default_get(fields)
    ...
    def action_create_suratsakit(self):
        domain = []
        ...
        return self.env.ref('om_hospital.surat_sakit_templates').report_action(self, data=data)
        # return self.env.ref('om_hospital.surat_sakit_templates').report_action(self, data=data)

class CreateSuratSakitWizReportVal(models.AbstractModel):
    _name ='report.om_hospital.surat_sakit_template'
